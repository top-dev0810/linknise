...
function getPasswordStrength(password: string) {
    ...
    if (password.length < 8) return { label: "Weak", color: "text-yellow-500" };
    if (password.match(/[A-Z]/) && password.match(/[0-9]/) && password.length >= 8) return { label: "Strong", color: "text-green-500" };
...
export default function SignUp() {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-[#10182a]">
            ...
            >
                <h1 className="text-3xl font-bold text-center text-white mb-2">Sign Up</h1>
                <button
                    type="button"
                ...
                <div className="w-full flex flex-col gap-4">
                    <label className="flex flex-col gap-1 text-sm text-white font-medium">
                        <span className="flex items-center gap-2"><span className="material-icons text-base">person</span>Full Name</span>
                        <input
                            type="text"
                            value={name}
                    ...
                    <label className="flex flex-col gap-1 text-sm text-white font-medium relative">
                        ...
                        {password && <span className={`text-xs font-semibold mt-1 ${strength.color}`}>{strength.label}</span>}
                    </label>
                </div>
                <button
                    type="submit"
