...
export default function CreateLinkPage() {
    ...
    function handleActionUrlChange(idx: number, value: string) {
        ...
        setUrlErrors(errors => {
            ...
            const action = unlockActions[idx];
            const error = validateActionUrl(idx, value, action.platform, action.type, action.label);
            updated[idx] = error;
            return updated;
        });
    }
    ...
    function validateActionUrl(idx: number, url: string, platform: string, type: string, label: string) {
        const validatorKey = `${platform}:${type}`;
        const validator = URL_VALIDATORS[validatorKey];
        if (!url) return '';
        if (validator && !validator(url)) {
            return `Not a valid ${platform} - ${label} URL`;
        }
        return '';
    }
                        ...
                        return (
                            <div key={idx} className="flex flex-col gap-3 mb-4 bg-[#181c1b] p-4 rounded-xl border border-gray-700 relative">
                                <div className="flex items-center gap-3">
                                    ...
                                    <button
                                        type="button"
                                        className={`flex items-center gap-2 px-4 py-2 rounded-lg text-white font-medium ${platform?.color || 'bg-gray-600'} hover:opacity-80 transition`}
                                    >
                                ...
                                {action.platform && action.type && (
                                    <div className="space-y-2">
                                        ...
                                        />
                                        {urlErrors[idx] && (
                                            <div className="text-xs text-red-500 font-medium mt-1">{urlErrors[idx]}</div>
                                        )}
                                        <div className="text-xs text-gray-400">
                                            Example: {getActionTypeOptions(action.platform).find(opt => opt.value === action.type)?.validationExample || "https://example.com"}
                                        </div>
                                        {urlErrors[idx] && (
                                            <div className="text-xs text-red-400 mt-1">Make sure your URL matches the format shown in the example above.</div>
                                        )}
                                    </div>
                                )}
                            </div>
